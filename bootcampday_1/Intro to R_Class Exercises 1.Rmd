...
rheumatoid_index <- which(arthritis == "rheumatoid")
arthritis[rheumatoid_index]
```

5) From the vector in the last question, count how many NA's are present. Use coding!
```{r}
na_count <- sum(is.na(arthritis))
na_count
```
...
less_than_10 <- sum(number_vector < 10)
less_than_10
```
...
count_condition <- sum(number_vector == 5 | number_vector > 5555)
count_condition
```
...
```{r}
# Create a vector with 5 items
my_vector <- c("apple", 42, 3.14, "banana", 100)

# Subset to contain only 3 items (first 3)
...
smoking_factor <- factor(smoking_status)
...
names(grocery_list) <- c("Fruits", "Vegetables", "Sweets")

# Access the second vector (Vegetables)
second_vector <- grocery_list[[2]]
second_vector

# Check the type of items in the second vector
class(second_vector)
```